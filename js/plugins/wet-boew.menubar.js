 /*
 * Menu bar v1.04 / Barre de menu v1.04
...
(function($) {
	...
	$.fn.menubar = function() {
							...
							if (e.keyCode === 27) {
								...
								if (e.keyCode == 37){
									if (parentHeader > 0){
										...
										//Set the focus to the previous top link
										prev = $elm.closest('.mb-sm-open').closest('li').prev('li').find('a:eq(0)');
										if (prev.length > 0) prev.focus();
										else $menu.children().eq($menu.children().length - 1).find('a:eq(0)').focus();
									}
									...
									if (parentHeader <  headerLinks.size() - 1){
										...
										next = $elm.closest('.mb-sm-open').closest('li').next('li').find('a:eq(0)');
										if (next.length > 0) next.focus();
										else $menu.children().eq(0).find('a:eq(0)').focus();
									}
								}
		...
		function hideSubMenu(topLink) {
			...
			$sm.attr("aria-expanded", "false").attr("aria-hidden", "true").toggleClass('mb-sm mb-sm-open').css("right","");
			allClosed = true;
		}
		...
		*/
		...
		if ($vbrumbs.size() > 0 && !$scope.hasClass('page-match-off')) {
			var $results = $menu.children('li').find('a:first').filter('[href="'+window.location.pathname+'"]');
			if ($results.size() > 0 ) {
				...
				if ($bcLinks.size() > 0) {
					for (i=0;i<=$bcLinks.size();i++) {
						$results = $menu.children('li').find('a:first').filter('[href="'+$bcLinks.eq(i).attr('href')+'"]');
						if ($results.size() > 0) {
							$results.eq(0).addClass('nav-current');
							match = true;
							break;
				...
				if (!match) {
					var bcText = $vbrumbs.find("li:last-child").text();
					if (/MSIE ((5\.5)|6|7)/.test(navigator.userAgent)) {
						$results = $menu.children('li').find('a:first').each(function() {
							if ($(this).children('.sublink') === bcText) {
								$results.eq(0).addClass('nav-current');
								return false;
							}
						});
					} else {
						$results = $menu.children('li').find('a:first').filter(':contains("' + bcText + '")');
